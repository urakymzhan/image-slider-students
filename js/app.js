...
// ** ULANS NOTE BELOW **
// deleted carNames. No need to output both: carOverlapName and carNames.
// ideally better use only one counter: either count or timerCount

var keysBgImgData = {};

// make sure bgImgData is not empty
if (bgImgData.length > 0) {
  bgImgData.forEach(function (item) {
    keysBgImgData[item.id] = item;
    var imgTab = document.createElement('img');
...
// inital data
imgContainer.style.background = `url(${bgImgData[0].imageUrl}) center/cover no-repeat`;
carOverlapName.textContent = bgImgData[0].name;
carouselContent.children[0].setAttribute('class', 'active');
...
containerDiv.addEventListener('click', function (event) {
  var imgId = event.target.getAttribute('data-id');

  // make sure data-id is not null
  if (imgId !== null) {
    setBgImageAndName(imgId);
  }

  // icon_right button
  if (event.target.getAttribute('id') === 'icon-right') {
    count++;
    setBgImageAndName(count);
    setActive(count);
  }
  if (count === bgImgData.length) {
    count = 0;
...
// helper function for playBtn
function setImgInterval() {
  timerCount++;
  setBgImageAndName(timerCount);
  setActive(timerCount);

  if (timerCount > bgImgData.length - 1) {
    timerCount = 0;
  }
}
// helper function to set background image and names
function setBgImageAndName(ind) {
  imgContainer.style.background =
    "url('" + keysBgImgData[ind].imageUrl + "') center/cover no-repeat";
  carOverlapName.textContent = keysBgImgData[ind].name;
}
// helper function to set css active class
function setActive(ind) {
  for (var item of carouselContent.children) {
    item.classList.remove('active');
  }
  carouselContent.children[ind - 1].setAttribute('class', 'active');
}
